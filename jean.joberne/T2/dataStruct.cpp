...
std::istream& jean::operator>>(std::istream& in, DataStruct& data)
{
...
std::istream& jean::operator>>(std::istream& in, std::string& exp)
{
  std::istream::sentry guard(in);
  if (!guard)
  {
...
bool jean::DataStruct::operator<(const DataStruct& data) const
{
  if (key1 == data.key1)
  {
